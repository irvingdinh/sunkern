...
func TestGetIntRejectsFractionalJSONNumber(t *testing.T) {
	loadWithJSON(t, `{"port": 8080.5}`)

	if got := GetOr[int]("port", 19110); got != 19110 {
		t.Errorf("port = %d, want 19110 (fractional JSON number fallback)", got)
	}
...
func TestGetUintRejectsFractionalJSONNumber(t *testing.T) {
	loadWithJSON(t, `{"limit": 12.5}`)

	if got := GetOr[uint]("limit", 99); got != 99 {
		t.Errorf("limit = %d, want 99 (fractional JSON number fallback)", got)
	}
...
func TestGetDuration(t *testing.T) {
	...
	// Typed duration defaults remain supported.
	SetDefault("timeout", 30*time.Second)
	if got := Get[time.Duration]("timeout"); got != 30*time.Second {
		t.Errorf("timeout = %v, want 30s", got)
	}
}

func TestGetDurationRejectsNumericValues(t *testing.T) {
	loadWithJSON(t, `{"timeout": 30}`)

	if got := GetOr[time.Duration]("timeout", 5*time.Second); got != 5*time.Second {
		t.Errorf("timeout = %v, want 5s (numeric JSON duration fallback)", got)
	}

	t.Setenv("RETRY_AFTER", "30")
	if got := GetOr[time.Duration]("retry_after", 10*time.Second); got != 10*time.Second {
		t.Errorf("retry_after = %v, want 10s (bare numeric string fallback)", got)
	}

	mustPanic(t, "cannot coerce", func() {
		Get[time.Duration]("timeout")
	})
}
...
func TestSubAcceptsTrailingDot(t *testing.T) {
	loadEmpty(t)
	SetDefault("jwt.secret", "top-secret")

	sub := Sub("jwt.")
	if got, ok := sub["secret"]; !ok || got != "top-secret" {
		t.Errorf("sub[secret] = %v, want %q", got, "top-secret")
	}
}
