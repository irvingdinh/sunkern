...
**Last session**: 2026-03-25 — Session 47 (framework/app revisited — ModuleInfo children, boot-order validation, HealthCheckProvider)
**Working tree**: clean
...
|---------|----------|--------------|-------|
| `framework/app` | **Maturing** | Session 47 | Revisited: ModuleInfo.Children recursive expansion for ModuleGroup introspection (admin dashboards see full nested module tree), buildModuleInfo helper with parentDisabled propagation (disabled groups mark all children disabled, metadata preserved). Boot-order validation in validateDependencies (position index checks dependency registered before dependent — catches wrong module ordering, not just missing deps). HealthCheckProvider optional interface (modules auto-register health checks during boot without resolving *App, recurses into ModuleGroup children). 9 new tests (49 total). Load tested: ModuleInfo 69K req/s p99 2.5ms, Health 58K req/s p99 2.6ms, 28 MB RSS. Zero data races. Previous (Session 38): Tagger, DependencyDeclarer, When(), ModuleStatus/ModuleInfo, duplicate name validation. Previous (Session 30): PostBooter, PreShutdowner, phase timing, ModuleGroup delegates. Previous (Session 25): health check system, ReadyCh, Env/Version. Previous (Session 13): *App supplied to container, boot/shutdown timing. Previous: ModuleGroup boot rollback fix |
| `framework/container` | **Maturing** | Session 46 | Revisited: exported container-level generic functions for isolated containers (ProvideTo, SupplyTo, MakeFrom, MustMakeFrom, HasIn, OverrideIn, OverrideSupplyIn — enables testing without touching global state), BuildAll() eagerly resolves all pending providers in sorted order (fail-fast at boot instead of lazy first-request failures, returns errors.Join of all failures), buildByName non-generic resolver shares circular detection and dependency tracking with makeFromContainer, runProvider extracted as shared core (eliminates code duplication between makeFromContainer and buildByName), captureCallerAt(skip) generalized caller capture (DRY — both global facade and exported functions use it), 29 new tests (77 total). Load tested: Inspect 68K req/s p99 2.5ms, DependencyGraph 68K req/s p99 2.4ms, Keys 70K req/s p99 2.4ms, BuildAll (3 deps) 11K req/s p99 31ms, per-request container stress 11K req/s p99 31ms, 36 MB RSS. Zero data races. Previous (Session 37): circular dependency detection, dependency tracking, DependencyGraph, Container.Reset. Previous (Session 26): ServiceInfo enriched with Kind/Caller/ErrorText; HookReport. Previous (Session 13): introspection APIs. Previous: Override/OverrideSupply, named hooks |
...
| `framework/http` | **Maturing** | Session 44 | Session 44: SkipMethods(mw, methods...) utility added to chain.go — method-based middleware bypass, complements SkipPaths, 3 new tests (30 total in http package). Previous (Session 43): SSE Broker — topic-based fan-out hub managing multiple SSE connections (NewBroker, Run, Subscribe, Publish, Broadcast, Stats). Single event-loop goroutine (channel-based, no locks for client/topic maps), per-client buffered channels with non-blocking drops for slow clients, atomic stats counters. Options: WithHeartbeatInterval, WithClientBuffer. ErrBrokerStopped sentinel. ResponseRecorder — exported response inspection wrapper. Load tested (20 SSE clients): stats 71K req/s, POST+publish 21K req/s. Previous (Session 35): Chain, SkipIf, SkipPaths. Previous (Session 28): PaginationParams, embedded BindQuery, Created(), configurable timeouts. Previous (Session 14): SSE support. Previous (Session 10): Bind/BindForm MaxBytesError → 413 |
| `framework/http/middleware` | **Maturing** | Session 44 | Revisited: CSRF middleware (double-submit cookie — constant-time comparison, configurable cookie/header names/path/secure/samesite, auto-token generation on missing/invalid cookie, safe methods exempt), CacheBody middleware (request body buffering with size limit, replayable r.Body + context storage via CachedBody(r), 413 on oversized), SkipMethods utility in chain.go (method-based middleware bypass, complements SkipPaths). 18 new tests (46 total middleware, 30 total http). Load tested: CSRF-validated POST 25K req/s p99 8.5ms, CacheBody+HMAC POST 25K req/s p99 8.4ms, CSRF rejection 68K req/s p99 2.7ms, GET list with SkipMethods 5.2K req/s p99 25ms (5K rows), CacheBody 413 rejection 5.7K req/s p99 9ms, 99 MB RSS. Zero data races. Previous (Session 35): SubjectFromCtx, RoleFromCtx. Previous (Session 28): writeErrorJSON shared helper. Previous (Session 15): Timeout, PBKDF2-SHA256, APIToken, GenerateToken. Previous (Session 10): JWT, Auth, RequireRole, MaxBytes. Plus existing: RequestID, RequestLogger, Recover, CORS, RateLimit |
| `framework/db` | **Maturing** | Session 45 | Revisited: UpdateByID(table, id) convenience returning pre-configured UpdateBuilder with WHERE id = ? AND deleted_at IS NULL (completes the convenience set: FindByID, DeleteByID, SoftDeleteByID, RestoreByID, UpdateByID), service/users controller updated to use it. BatchUpdate(table, keyCol) — CASE-based multi-row UPDATE builder for bulk operations. Row(keyVal).Set(col, val) fluent API, supports mixed column sets per row (ELSE preserves existing value), auto-sets updated_at, WHERE key_col IN (...). Row().Row() chaining for fluent batch construction. Load tested with inventory app (1000 products): UpdateByID 28K req/s p99 7.5ms, batch-update 5 rows 28K req/s p99 7.7ms, batch-update 50 rows 4.6K req/s p99 48ms, batch-restock 20 rows 9.4K req/s p99 21ms, 30.5 MB RSS. Zero data races. Previous (Session 36): RunTxVal[T], Exists() optimized, FindByID[T], DeleteByID. Previous (Session 27): INSERT...SELECT, DoUpdateAll, ConflictBuilder.Where, ConflictWhere, Excluded(), Returning, ReturningStar. Previous (Session 24): CTEs. Previous (Session 23): set operations, FILTER, Query interface. Previous (Session 22): window functions. Previous (Session 21): JOIN ergonomics. Previous (Session 12): RETURNING, subqueries, CASE. Previous (Session 5): nullable types, cursor pagination |
...
Session 44 (middleware revisit, webhook receiver with CSRF + CacheBody + SkipMethods, 5K+ rows):
- [csrf/POST validated] 25,240 req/s, p99 8.5ms — CSRF cookie + header validation + DB INSERT
- [csrf/POST rejected] 68,170 req/s, p99 2.7ms — CSRF rejection path (no token, 403)
- [cachebody/POST hmac] 25,474 req/s, p99 8.4ms — CacheBody buffer + HMAC-SHA256 verify + DB INSERT
- [cachebody/POST 413] 5,692 req/s, p99 9.0ms — 1.1 MiB body, rejected at 413 (IO-bound by body transfer)
- [skipmethods/GET list] 5,260 req/s, p99 25ms — paginated list on ~5K rows, CSRF bypassed by SkipMethods
- [memory] 99 MB RSS after 25K+ webhook writes under sustained load
- [race] No data races detected with -race flag on all middleware and http tests

Session 45 (db revisit, inventory app with UpdateByID + BatchUpdate, 1000 products):
- [db/GET list] 15,949 req/s, p99 7.8ms — paginated list on 1000 products
- [db/PATCH UpdateByID] 28,300 req/s, p99 7.5ms — single row update via UpdateByID convenience
- [db/POST batch-update 5] 28,188 req/s, p99 7.7ms — BatchUpdate CASE with 5 rows, 3 columns
- [db/POST batch-update 50] 4,587 req/s, p99 47.6ms — BatchUpdate CASE with 50 rows, 3 columns
- [db/POST batch-restock 20] 9,393 req/s, p99 21.4ms — BatchUpdate single column, 20 rows + read-back
- [memory] 30.5 MB RSS under sustained load
- [race] No data races detected with -race flag on concurrent batch + read operations

Session 46 (container revisit, task app with container introspection + BuildAll, 1000 tasks):
- [container/GET Inspect] 67,952 req/s, p99 2.5ms — global container Inspect() (8 services)
- [container/GET DependencyGraph] 68,320 req/s, p99 2.4ms — global container DependencyGraph()
- [container/GET Keys] 70,503 req/s, p99 2.4ms — global container Keys()
- [container/POST BuildAll] 10,869 req/s, p99 30.7ms — isolated container, 3 services with deps, BuildAll + inspect
- [container/POST BuildAll-fail] 30,838 req/s, p99 9.3ms — isolated container, BuildAll with intentional error
- [container/POST stress] 10,888 req/s, p99 31.1ms — per-request isolated container (3 deps chain, BuildAll + graph)
- [memory] 36 MB RSS under sustained load (50K+ requests across all endpoints)
- [race] Zero data races under 100-concurrency mixed workload (20K container ops + 5K CRUD, concurrent)

Session 47 (app revisit, task tracker with ModuleGroup introspection + HealthCheckProvider, 500 tasks):
- [app/GET ModuleInfo] 68,554 req/s, p99 2.5ms — ModuleInfo() with 3 top-level modules (1 ModuleGroup with 2 children), recursive Children expansion
- [app/GET Health] 58,314 req/s, p99 2.6ms — CheckHealth() with 3 auto-registered providers (sqlite + heartbeat + tasks-db)
- [admin/GET stats] 5,862 req/s, p99 ~10ms — GROUP BY aggregation on 500 rows
- [core/GET heartbeat] 68,374 req/s, p99 2.5ms — atomic counter read
- [memory] 28 MB RSS under sustained load (60K+ requests across all endpoints)
- [race] Zero data races under 70-concurrency mixed workload (modules + health + CRUD, concurrent)
...
- [middleware] CSRF uses double-submit cookie pattern — token stored in a non-HttpOnly cookie (JS must read it) and validated against X-CSRF-Token header. Constant-time comparison via subtle.ConstantTimeCompare prevents timing attacks. Safe methods (GET, HEAD, OPTIONS, TRACE) are exempt — no need for SkipMethods in most cases. (Session 44)
- [middleware] CSRF cookie is always set on every request (even rejected ones) to ensure the client can obtain a token for retry. Invalid-length cookies trigger token regeneration. Token is 32 random bytes, hex-encoded (64 chars) — same entropy as GenerateToken(). (Session 44)
- [middleware] CacheBody reads the full body into memory and replaces r.Body with a bytes.NewReader (replayable). The cached bytes are also stored in the request context via cachedBodyKey. The maxSize+1 read pattern detects oversized bodies without allocating maxSize+1 bytes (LimitReader caps at maxSize+1, then length check reveals overflow). (Session 44)
- [middleware] CacheBody stores []byte in context (shared, not copied) — the CachedBody() accessor documents "do not modify." Copying on every access would be safer but wasteful for the primary use case (HMAC verification, logging). (Session 44)
- [http] SkipMethods is a convenience wrapper around SkipIf, mirroring SkipPaths. Uses map for O(1) method lookup. Zero methods = return original middleware (same pattern as SkipPaths with zero paths). (Session 44)

- [db] UpdateByID returns UpdateBuilder with WHERE id = ? AND deleted_at IS NULL — consistent with FindByID/SoftDeleteByID/RestoreByID which all filter by id + not-deleted. The caller chains .Set() and .Exec() (can't execute immediately like DeleteByID since SET columns vary). (Session 45)
- [db] BatchUpdate uses CASE expressions per column, not per row. Each column gets one CASE with WHEN clauses for each row that sets it, plus ELSE preserving the existing value. This means rows that don't set a column are simply absent from that column's CASE — no NULL injection, no overwrite. Single UPDATE statement regardless of batch size. (Session 45)
- [db] BatchUpdate key column is WriteSQL'd in CASE (fully qualified "table"."col") but quoteIdent'd in WHERE IN (just "col"). Both are correct for single-table UPDATE — SQLite resolves the unqualified name in WHERE context. (Session 45)
- [db] BatchUpdate auto-appends updated_at = ? only if no row explicitly sets it. This parallels UpdateBuilder.Build() behavior. (Session 45)
- [db] BatchRow.Row(keyVal) chains back to the parent builder — allows fluent batch.Row("a").Set(...).Row("b").Set(...) without intermediate variable reassignment. (Session 45)

- [container] Exported generic functions use preposition naming: ProvideTo/SupplyTo (put into), MakeFrom/MustMakeFrom (get from), HasIn (check in), OverrideIn/OverrideSupplyIn (replace in). Reads naturally in English: `container.ProvideTo(c, fn)` = "provide T to c". Go generics can't be methods on *Container, so package-level functions are the only option. (Session 46)
- [container] captureCallerAt(skip) replaces the old captureCallerOf() — generalized to work with both global facade (skip=2: captureCallerOf→facade→caller) and exported functions (skip=1: captureCallerAt→ProvideTo→caller). Single implementation, no duplication. (Session 46)
- [container] runProvider extracted from makeFromContainer as the shared non-generic core. Caller MUST hold svc.mu. Handles pushResolve/popResolve, defer-based panic cleanup, error caching, dependency recording. Both makeFromContainer (generic, returns T) and buildByName (non-generic, used by BuildAll) delegate to it. Eliminates parallel code paths that could drift. (Session 46)
- [container] BuildAll iterates pending services in sorted alphabetical order (deterministic). If provider A resolves B internally (via MustMake), B is built on-demand through the normal path — BuildAll's iteration simply ensures nothing is left pending. Already-built services (including Supplied values) are skipped. (Session 46)
- [container] BuildAll uses errors.Join to return all provider errors. This differs from the first-error-stops pattern because some providers are independent — failing one shouldn't prevent building others. Admin can see all boot failures at once rather than fix-one-restart-fix-next. (Session 46)
- [container] buildByName includes full circular detection and dependency tracking, identical to makeFromContainer. The only difference: no typed return value. When called from BuildAll at top level, the resolution stack is empty so recordDep is a no-op — but when the provider calls MustMake internally, the full tracking activates. (Session 46)

- [app] ModuleInfo.Children is populated by buildModuleInfo, which recursively expands *ModuleGroup targets. The function accepts a parentDisabled flag — when a group is disabled via When(false, &ModuleGroup{...}), all its children inherit disabled status without individual disabledModule wrappers. Metadata (Tags, Dependencies) is extracted from the inner module even when disabled. (Session 47)
- [app] buildModuleInfo constructs per-group childBootedSet from g.booted (not a.booted). This correctly reflects the group's internal boot state — a.booted only contains the group itself, not its children. Children's appReady is gated on both the global appReady flag and the parent not being disabled. (Session 47)
- [app] Boot-order validation uses a position index (name → registration index) built once per validateDependencies call. For each module's DependsOn, it checks depPos > modPos — meaning the dependency was registered AFTER the dependent module (wrong order since registration order = boot order). Clear error message: "module X depends on Y, but Y is registered after X (dependencies must be registered first)". (Session 47)
- [app] HealthCheckProvider is checked after each module boots in boot() and recursively for ModuleGroup children via registerModuleHealthChecks. This means disabled modules (which skip boot) never register health checks — correct behavior since disabled modules shouldn't contribute to health status. (Session 47)

## Next Priorities

<!-- What the last session thinks should come next, in order -->

1. **Revisit `framework/config`** — last touched Session 39 (8 sessions ago, oldest). Future: config watcher (detect env var changes at runtime), config schema generation (JSON Schema from registered keys+rules+descriptions for external tooling), DescribeMany() bulk variant
2. **Revisit `framework/log`** — last touched Session 40. Future: mmap-based query for very large files (deferred), multi-date query (span across day boundaries), log entry struct tags for faster JSON extraction (avoid map[string]any), Query streaming via callback/channel for memory-bounded large result sets
3. **Revisit `framework/sqlite/migrate`** — last touched Session 41. Future: dual-pool support (read pool for Status/Validate/Pending queries, write pool for mutations), migration groups/batches (tag migrations, apply by group), migrate down to named migration (instead of version number)
4. **Revisit `framework/sqlite/driver`** — last touched Session 42. Future: blob I/O (sqlite3_blob_open/read/write/close for incremental large object access), sqlite3_authorizer (per-statement access control), connection-level update hook installation (currently only via Connector, needs pool recycling)
5. **Revisit `framework/sqlite`** — last touched Session 42. Future: expose WAL hook at DB level for custom callbacks (currently only used internally for checkpoint signaling), DB.OnChange(fn) high-level change notification API wrapping update hook with table filtering
6. **Revisit `framework/http`** — last touched Session 44. Future: Broker topic management (add/remove topics after subscribe), Broker event ID + replay buffer for reconnection recovery, Broker per-topic stats, Broker max-clients limit, ResponseRecorder with response body capture option
7. **Revisit `framework/http/middleware`** — last touched Session 44. Future: ETag/conditional GET, security headers (X-Content-Type-Options, X-Frame-Options, Referrer-Policy, HSTS), request ID forwarding to downstream services, CSRF per-session token rotation
8. **Revisit `framework/db`** — last touched Session 45. Future: query logging/tracing hook (leverage driver trace_v2 for expanded SQL + timing), BatchUpdate with expressions (SetExpr for increment/decrement in batch), prepared statement caching exploration, batch delete/soft-delete convenience
9. **Revisit `framework/container`** — last touched Session 46. Future: provider timeout (context-based deadline for lazy init), Invoke pattern (one-shot boot functions with dependency tracking)
10. **Revisit `framework/app`** — last touched Session 47. Future: module enable/disable at runtime via config change callback, boot timeout context (currently Boot has no deadline), ModuleGroup implementing Tagger/DependencyDeclarer directly (currently metadata only on individual modules)
