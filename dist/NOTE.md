...
**Last session**: 2026-03-25 — Session 46 (framework/container revisited — exported generic functions, BuildAll)
**Working tree**: clean
...
1. **Revisit `framework/app`** — last touched Session 38 (8 sessions ago, oldest). Future: module boot ordering via DependsOn (topological sort — currently validation only), ModuleGroup children introspection (nested ModuleInfo), module enable/disable at runtime via config change callback
2. **Revisit `framework/config`** — last touched Session 39. Future: config watcher (detect env var changes at runtime), config schema generation (JSON Schema from registered keys+rules+descriptions for external tooling), DescribeMany() bulk variant
3. **Revisit `framework/log`** — last touched Session 40. Future: mmap-based query for very large files (deferred), multi-date query (span across day boundaries), log entry struct tags for faster JSON extraction (avoid map[string]any), Query streaming via callback/channel for memory-bounded large result sets
4. **Revisit `framework/sqlite/migrate`** — last touched Session 41. Future: dual-pool support (read pool for Status/Validate/Pending queries, write pool for mutations), migration groups/batches (tag migrations, apply by group), migrate down to named migration (instead of version number)
5. **Revisit `framework/sqlite/driver`** — last touched Session 42. Future: blob I/O (sqlite3_blob_open/read/write/close for incremental large object access), sqlite3_authorizer (per-statement access control), connection-level update hook installation (currently only via Connector, needs pool recycling)
6. **Revisit `framework/sqlite`** — last touched Session 42. Future: expose WAL hook at DB level for custom callbacks (currently only used internally for checkpoint signaling), DB.OnChange(fn) high-level change notification API wrapping update hook with table filtering
7. **Revisit `framework/http`** — last touched Session 44. Future: Broker topic management (add/remove topics after subscribe), Broker event ID + replay buffer for reconnection recovery, Broker per-topic stats, Broker max-clients limit, ResponseRecorder with response body capture option
8. **Revisit `framework/http/middleware`** — last touched Session 44. Future: ETag/conditional GET, security headers (X-Content-Type-Options, X-Frame-Options, Referrer-Policy, HSTS), request ID forwarding to downstream services, CSRF per-session token rotation
9. **Revisit `framework/db`** — last touched Session 45. Future: query logging/tracing hook (leverage driver trace_v2 for expanded SQL + timing), BatchUpdate with expressions (SetExpr for increment/decrement in batch), prepared statement caching exploration, batch delete/soft-delete convenience
10. **Revisit `framework/container`** — last touched Session 46. Future: provider timeout (context-based deadline for lazy init), service health integration (register health checks from providers automatically), Invoke pattern (one-shot boot functions with dependency tracking)
